...
void _testProfile(){
  ...
  var name = html
        .getElementsByClassName("pv-text-details__title")[0]
      .getElementsByTagName("h1")[0].text;
  print("✅ name: $name");


  var jobTitle = html
      .getElementsByClassName("artdeco-list__item pvs-list__item--line-separated pvs-list__item--one-column")[0] .getElementsByClassName("display-flex flex-row justify-space-between")[0]
  ...
  var companyNameAndContractType = html
      .getElementsByClassName("artdeco-list__item pvs-list__item--line-separated pvs-list__item--one-column")[0] .getElementsByClassName("display-flex flex-row justify-space-between")[0]
      .getElementsByClassName("visually-hidden")[1].text;
  print("✅ companyNameAndContractType: $companyNameAndContractType");

  var thoigianlamviec = html
      .getElementsByClassName("artdeco-list__item pvs-list__item--line-separated pvs-list__item--one-column")[0] .getElementsByClassName("display-flex flex-row justify-space-between")[0]
      .getElementsByClassName("visually-hidden")[2].text;
  ...
  String getProfileUrl(String url) {
   ...
   for(int i = 0;i < items.length; i++){
    try{
     ...
     users.add(LinkedinUserModel(name: name ?? '',avatar: avatar ??'',url: getProfileUrl(link ?? '')));
    }catch(e){
     continue;
    }
   }
